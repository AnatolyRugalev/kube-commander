...
- **Docker builds were dropped on 2026-08-09 (D254)** — no `Dockerfile`, no `dockers_v2:`,
  no container install path. The goreleaser/CI docker knowledge that used to live here
  (buildx temp build context, `ARG TARGETPLATFORM` + `COPY $TARGETPLATFORM/<bin>`,
  docker-container driver, sandbox `dockerd`) is preserved in git history and in D184's
  superseded text; re-derive it there if containers ever come back, don't rebuild from
  memory.
...
**Docker in the sandbox (M5-08, historical).** The `docker` *client* was on PATH with **no
daemon** — `dockerd &` started one (running as root) and then everything worked: pulls
reached `gcr.io`/`docker.io` through the proxy, buildx built both arches of a `COPY`-only
Dockerfile with no QEMU, and `--load`ed images ran. That let the Docker slice be the one
distribution path an agent could verify end to end. Since D254 dropped the container image,
this is here for context only — do not resurrect the workflow steps on its strength.

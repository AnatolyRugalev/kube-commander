...
_Last updated: 2026-07-18 — M1-06c done: **cordon/uncordon** actions (`internal/kube/actions.go`) — `Clients.Cordon`/`Clients.Uncordon(ctx, r, ref)` merge-patch a node's `spec.unschedulable` (true/false) through the dynamic client, as `kubectl cordon`/`uncordon` do; cluster-scoped (ref ns dropped); uncordon writes concrete `false` not null; idempotent → no UID guard; NotFound wrapped (#86); D37. **M1-06c narrowed** from "cordon/uncordon + drain" — drain split to new **M1-06e** (eviction API is its own green leg). M1-06 progress: 06a delete + 06b scale/restart + 06c cordon/uncordon done; **06d cronjob-suspend**, **06e drain** remain. Active milestone: M1; next up **M1-06d**. M1-04b (lazy group detail) parked until the M2 menu exists._

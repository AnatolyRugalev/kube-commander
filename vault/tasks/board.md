...
_Last updated: 2026-07-18 — M1-06a done: generic **delete** action (`internal/kube/actions.go`) — `Clients.Delete(ctx, Resource, ObjectRef, DeleteOptions)` removes any resource (built-in/CRD) through the dynamic client via a shared `resourceInterface(r, ns)` helper; layers a UID precondition (pure `withUIDPrecondition`) so acting on a table snapshot never deletes a recreated same-named object; NotFound surfaced wrapped; D35. M1-06 was split (too big, cf. D33): 06a delete done; **06b scale+rollout-restart**, **06c cordon/drain**, **06d cronjob-suspend** remain. Active milestone: M1; next up **M1-06b**. M1-04b (lazy group detail) parked until the M2 menu exists._

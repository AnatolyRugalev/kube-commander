...
- [ ] **STORY-06h-1** The viewer fills the right pane — describe (and every shared-viewer surface: YAML, secret, events) takes the whole right pane instead of a centered inset, so a describe dump has real room to read (`2026-08-15-describe-replaces-right-pane.md`)
      status: in-progress | owner: claude-opus | added: 2026-08-20
      notes: First slice of STORY-06h, split on pickup (layout before paint — the panel needs the room before it can carry colour). The viewer is the one shared pager (`components/viewer`) behind describe/YAML/secret/events, so the change is one component + the shell's composite: drop the box's screen margin, size it to the table pane's geometry, and composite it at the pane's origin rather than centering. The menu pane stays visible beside it (a hidden menu hands it the full width, exactly as the table gets it). D95 (popups overlay the browse view) keeps holding for the *modals* — help, pickers, confirm; a pager is not a popup. The feedback file is deleted with this slice (D69).

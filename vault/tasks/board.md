...
- [ ] **STORY-06k-1** Single enter reaches a search hit — `enter` opens the highlighted hit from the query line itself, and the hand-off into the result list (where `hjkl` navigate) moves to the arrow that already meant "down the list" (`2026-08-15-search-single-enter.md`)
      status: in-progress | owner: claude-opus | added: 2026-08-20
      notes: First slice of STORY-06k, split on pickup (S05's two search frictions, smallest first). The view already emits SelectedMsg on one drill-in; the friction is SEARCH-05's commit step (D235), where the first enter only moves focus. Supersede that half: drillIn opens whatever is highlighted regardless of focus, and a list movement from the query field is what commits. The preview (06k-2) keeps its own feedback file (D69).
      → milestone: M5 · knowledge: decisions.md D268, D235

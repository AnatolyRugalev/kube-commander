...
_Last updated: 2026-07-19 — M1-06e-1 done: **drain pod selection** (`internal/kube/drain.go`) — `Clients.DrainCandidates(ctx, node, opts)` lists a node's pods (typed clientset, `spec.nodeName` selector) and runs a pure `classifyDrainPods` mirroring `kubectl drain`: skip mirror/terminated/DaemonSet(w/ IgnoreDaemonSets); block (one aggregated refusal, nil slice) standalone-w/o-Force, DaemonSet-w/o-Ignore, emptyDir-w/o-DeleteEmptyDirData; evict controller-managed. Returns []ObjectRef for the eviction step; empty node rejected, list error wrapped (#86); D39. Drain uses the typed clientset (pod/node-specific), a departure from 06a–06d's dynamic path. M1-06 progress: 06a–06d done; 06e-1 selection done, only **06e-2 eviction loop** remains. Active milestone: M1; next up **M1-06e-2**, then **M1-07** (logs/describe/YAML). M1-04b (lazy group detail) parked until the M2 menu exists._

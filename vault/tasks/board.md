...
_Last updated: 2026-07-30 — M5-05 verified the migration path against a legacy config generated by the 2020 writer and bound to `master:pb/config.proto` (D180), so **M5-09 (the vhs tape) is the only unblocked M5 board item left** — M5-06/07/08 each need a human-owned secret and M5-10/11 wait on the rest. Seven human-tasks open: one **blocking** (the CRD error text, blocking CRD-01) and six advisory, of which the Edit dogfood carries two DoD boxes and the new legacy-config one carries an M5 exit criterion. Per-leg history: `vault/journal/`._

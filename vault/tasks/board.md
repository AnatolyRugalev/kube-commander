...
_Last updated: 2026-07-22 — M2-11a landed: config write-back primitives (`Config.Save`/`SaveFile`, atomic + 0o600, D89); M2-11 split so its remainder (M2-11b: last-namespace persistence + load-on-start) is the next config leg, its menu-customization half already covered by the per-context menu files (D83). Per-leg history: `vault/journal/`._

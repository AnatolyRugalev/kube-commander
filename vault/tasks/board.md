...
Reopened once, by STORY-06h-1's composite:
...
- [x] **STORY-06h-1** The viewer fills the right pane — describe (and every shared-viewer surface: YAML, secret, events) takes the whole right pane instead of a centered inset, so a describe dump has real room to read (`2026-08-15-describe-replaces-right-pane.md`)
      status: done | owner: claude-opus | added: 2026-08-20 | done: 2026-08-20
      notes: First slice of STORY-06h, split on pickup (layout before paint). Landed as D284: the viewer's box is now exactly the area it is sized to (`screenMargin`/`minWidth`/`minHeight` are gone), `resize` hands it the table pane's geometry, and `View` composites it with a new `overlayAt` at the pane's origin instead of centering — describe/YAML/secret/events all ride the one shared pager, so all four fill the pane, and a hidden menu hands it the full width exactly as it does the table. D95 keeps governing the modals (help, pickers, confirm, the port-forward panel): a popup's content is *about* the thing behind it, a pager's content is the thing being read. Pt 3 is the gotcha the composite exposed: the menu's frame renders **two columns narrower** than the width resize sizes it to, so the boundary is `rightPaneX()` (the rendered menu width), never `menuPaneWidth`; the 2-column dead gap that quirk leaves at the body's right edge is **BOX-04**. README's inspect paragraph names the layout; feedback file deleted (D69).
      → milestone: M5 · knowledge: decisions.md D268, D284

- [x] **STORY-06h-2** The painted describe panel — the describe output rendered as a styled panel: phase/status/conditions in theme-aware colours, problem states (CrashLoopBackOff, ImagePullBackOff, Unschedulable, unready) emphasized so the failing part is where the eye lands first (`2026-08-15-rich-describe-panel.md`)
      status: done | owner: claude-opus | added: 2026-08-20 | done: 2026-08-21
      notes: Second and last slice of STORY-06h, so 06h is closed. Landed as D285: `viewer.PaintDescribe` re-renders the dump styled — muted keys, accented section headings, and the state-carrying fields (Status/State/Last State/Phase/Reason/Ready/Restart Count, mapped onto classifier columns by `describeKeyColumns`) painted with the role `table.ClassifyValue` reads out of them. That function is the M4-06 cell classifier exported for surfaces that are not tables (`table.Role`), so the panel, the browse table's colouring and the `H`/`U` predicates share one vocabulary; a key not in the map leaves its value plain, which is what keeps `sidecar.istio.io/inject: false` from reading as a failed condition. The Conditions/Events blocks are whitespace-aligned rows, not fields, so each field is classified alone and only warn-or-worse is painted ("warning" joins `statusWords`). Painting is text→text and `ansi.Strip` of the output is the dump byte for byte — no reflow. `SetDescribeContent` keeps the raw dump so a runtime theme switch repaints in place at the same scroll offset. README's inspect paragraph names the colouring; feedback file deleted (D69).
      → milestone: M5 · knowledge: decisions.md D268, D285

- [x] **STORY-06i-1** The relation graph primitive — `kube.Relations` resolves one object into its navigable neighbours (owners from `ownerReferences`, the existing child scope, and a pod's spec-named links: node, service account, claims, config maps, secrets), each carrying the discovered target `Resource` (`2026-08-15-relations-navigation-popup.md`)
      status: done | owner: claude-opus | added: 2026-08-21 | done: 2026-08-21
      notes: First slice of STORY-06i, split on pickup (D52: the primitive before the surface), with 06i-2/06i-3 split back to Backlog. Landed as D286: `kube.Relations(ctx, res, ref, kinds)` costs exactly one Get and derives everything else from what it returned — `ownerReferences` gives the reverse of `res.children` (pod → ReplicaSet → Deployment, the headline ask, UID carried), `childScope` gives the forward direction off the same object (`Children` is now the fetch-then-`childScope` wrapper), and the spec names the rest: a pod's node / service account / PVC / config-map / secret volumes / image-pull secrets, a claim's volume, a volume's claim (its `claimRef` namespace winning). A relation is **named** (`Ref()`) or **set-shaped** (`Scope()`, the `ChildScope` drill-in already consumes) — two shapes, so the surface reuses two paths it has. A target kind missing from the caller's discovered set is dropped, never synthesized; an object with no neighbours is an empty slice, not an error; duplicates collapse on (role, kind, ns, name, selector). Kube-layer only, so the binary is unchanged. The feedback file stays until 06i-2 lands the popup (D69).
      → milestone: M5 · knowledge: decisions.md D268, D286

- [x] **STORY-06i-2** The relations popup — one gesture on any resource opens the list `kube.Relations` returns, each row navigable: a named relation switches browse to that kind with the object selected, a set-shaped one opens the child scope (`2026-08-15-relations-navigation-popup.md`)
      status: done | owner: claude-opus | added: 2026-08-21 | done: 2026-08-21
      notes: Second slice of STORY-06i and the one that **deletes the feedback file** (D69). Landed as D287: the gesture is `gr` — vim's goto family, where "go to references" already lives — because the mnemonic letter is spent three times over (`R` resources.switch, `r` secret.reveal, `P` res.children) and `g` already buffers for `gg`, so the sequence costs no key its own timeout; it is registered as a **row action** (`Related resources`, gated on `get`), so key, actions menu and palette dispatch one intent. A `Relater` seam joins the `Cluster` bundle (nil → inert, like `Searcher`/`Scanner`) and the list is a `picker` stamped `relation` — a modal over the browse view, which is the side of D95 that centers. Rows are stably sorted up → down → side with the direction's arrow in the name column (a picker has no section headings) and labelled by the target (`Kind/name`, or `Kind (selector)` for a scope), namespace-qualified when it differs from the browsed scope; the label is the pick's identity, so same-target rows collapse. A pick reuses a path the shell already has: named → `selectResource` + the pending selection the search drill-in arms, set-shaped → `selectChildScope` with the popup's source object as owner (so the scope label names it and `nav.back` returns to it). A cross-namespace named target re-scopes display state only, never persisted, and an all-namespaces scope is left alone. No neighbours is a notice, a failed resolve a toast. README + `docs/keybindings.md` updated; hermetic tests only — the row set on a real pod is for the next dogfood.
      → milestone: M5 · knowledge: decisions.md D268, D287
...
- [x] **STORY-06j-3** The newest-first log order weighed — considered and declined — done 2026-08-21 (D289)
      status: done | owner: claude-opus-5 | added: 2026-08-16 | done: 2026-08-21
      notes: Third slice of STORY-06j, and a decision leg — the feedback asked to be *considered*, not done. Declined as D289, no reversal and no config option. Half the ask is already true (the filter field renders above the body, and the view opens following with the cursor on the newest line, so a tailing full-screen pane already shows the fresh line first), and the residual pain is what D147/D281 + the `[following]` badge already answer. Against it: it would invert the direction of every nav key in one view (goals principle 6, D147 pt 3 — a view may add an effect to `G`, not redefine its direction); grafana reverses *events*, while this pager addresses lines (D242 pt 1), so a reversal prints every stack trace backwards; and it would split the screen from the clipboard (D242 pt 4) and two-branch `trim`/`atNewest`/`scrollDown`/`shownIdx`/selection/wrap arithmetic forever. Reopen condition recorded: event grouping (a structured-log mode with addressable multi-line records), not taste. No code change; feedback `2026-08-15-logs-newest-first-order.md` deleted (D69). STORY-06j is closed.
      → milestone: M5 · knowledge: decisions.md D268, D289
...
- [x] **STORY-06l** The two startup defaults — land on Pods, hold back unpinned CRDs — done 2026-08-21 (D288)
      status: done | owner: claude-opus-5 | added: 2026-08-15 | done: 2026-08-21
      notes: Landed as D288. A launch or context switch with nothing recorded opens the Pods table through the ordinary discovery-gated restore (`defaultLandingResource`; a remembered kind still wins, the welcome pane is the degrade, `WithLastResource` stays the opt-in seam). The resources pane holds back a discovered `Custom Resources` row with neither an authored `menus/<context>.yaml` entry (`Item.Authored`) nor a pin behind it, in `applyFilter` only — `Items()` keeps every kind, so the picker, `:pin`, cluster search, the unhealthy sweep, relations and pane memory are untouched. A `/` query reveals them, the open kind is always listed (`SetActive` re-derives), and a trailing `+N custom · / to find` row reports the count; `AddPinned` marks a row discovery already listed (else pinning a CRD was a no-op) and `Unpin` re-derives. The displayed list is no longer index-addressable, so pane memory uses the new `menu.SelectResource(gvr)`. Feedback `2026-08-15-land-on-pods-by-default.md` and `2026-08-15-hide-custom-resources.md` deleted (D69).
      → milestone: M5 · knowledge: decisions.md D268, D288
...
- [x] **STORY-06j-3** The newest-first log order weighed — considered and declined — done 2026-08-21 (D289)
- [x] **STORY-06l** The two startup defaults — land on Pods, hold back unpinned CRDs — done 2026-08-21 (D288)
- [x] **STORY-06i-2** The relations popup — `gr` (and the `Related resources` row action) lists the selected object's neighbours grouped by direction, each row opening its named object (browse + pending select) or its scope (the child drill-down) — done 2026-08-21 (D287)
- [x] **STORY-06i-1** The relation graph primitive — `kube.Relations` resolves an object's navigable neighbours in one Get: `ownerReferences` parents, the child scope, and the spec-named links (node, service account, claims, config maps, secrets), each either named or set-shaped — done 2026-08-21 (D286)
- [x] **STORY-06h-2** The painted describe panel — the describe dump renders styled through `table.ClassifyValue` (the M4-06 classifier exported), so status/state/reason/ready and the Conditions/Events rows carry the browse table's own colours and the failure is what the eye lands on — done 2026-08-21 (D285)
- [x] **STORY-06h-1** The viewer fills the right pane — the shared pager (describe/YAML/secret/events) is sized to the table pane and composited at its origin instead of centering as an inset, so a describe dump gets the pane whole while the menu stays beside it — done 2026-08-20 (D284)

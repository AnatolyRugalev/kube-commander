...
_Last updated: 2026-07-18 — M1-05b done: server-side Table **Watch** (`internal/kube/watch.go`) — `Clients.Watch` runs a reconnecting List→Watch goroutine that streams `WatchEvent{ADDED/MODIFIED/DELETED/RESET/ERROR}` on a bounded channel; opens the same request as List with `watch=true` via `.Stream()`, decodes the `metav1.WatchEvent` stream with `decodeTableRV`, caches columns per connection, resumes off `resourceVersion` (bookmarks), and re-Lists on `410 Gone` (sentinel `*errExpired`) emitting a fresh RESET; no new deps; D34. M1-05 (05a List + 05b Watch) now complete — M1 "List+watch any resource" exit criterion met. Active milestone: M1; next up M1-06 (actions: delete/scale/rollout-restart/cordon/drain/cronjob-suspend). M1-04b (lazy group detail) parked until the M2 menu exists._

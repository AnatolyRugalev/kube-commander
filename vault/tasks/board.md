...
_Last updated: 2026-07-18 — M1-06b done: **scale + rollout-restart** actions (`internal/kube/actions.go`) — `Clients.Scale(ctx, r, ref, replicas)` merge-patches the `scale` subresource (`spec.replicas`, generic over Deployment/RS/StatefulSet/RC + CRDs); `Clients.RolloutRestart(ctx, r, ref)` merge-patches `spec.template.metadata.annotations[kubectl.kubernetes.io/restartedAt]` with a UTC RFC3339 stamp (kubectl's exact key → interoperable). Merge (not strategic) patch keeps both schema-free for CRDs; no UID guard (idempotent, PatchOptions has no preconditions); D36. M1-06 split (D33): 06a delete + 06b scale/restart done; **06c cordon/drain**, **06d cronjob-suspend** remain. Active milestone: M1; next up **M1-06c**. M1-04b (lazy group detail) parked until the M2 menu exists._

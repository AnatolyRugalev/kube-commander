...
- [ ] **STORY-06i-1** The relation graph primitive — `kube.Relations` resolves one object into its navigable neighbours (owners from `ownerReferences`, the existing child scope, and a pod's spec-named links: node, service account, claims, config maps, secrets), each carrying the discovered target `Resource` (`2026-08-15-relations-navigation-popup.md`)
      status: in-progress | owner: claude-opus | added: 2026-08-21
      notes: First slice of STORY-06i, split on pickup (D52: the primitive before the surface). Kube-layer only, no TUI: one Get of the object, then pure derivation — `ownerReferences` gives the reverse of `res.children` (pod → its ReplicaSet → its Deployment, the headline ask), `childLinks` gives the forward direction it already had, and a pod's `spec` names the rest. A relation is either **named** (one object) or **set-shaped** (a `ChildScope`), so the surface can reuse the drill-in path the shell already has. Targets absent from the caller's discovered kind set are dropped, never synthesized. The feedback file stays until 06i-2 lands the popup (D69).
      → milestone: M5 · knowledge: decisions.md D268

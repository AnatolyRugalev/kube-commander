...
_Last updated: 2026-07-21 — draining the dogfood feedback inbox; FB-help-popup (the `?` help/keys view is now a centered, bordered modal box over the browse layout — like the picker — instead of a full-screen page; `q` also dismisses it) landed. One feedback item remains: 08 mouse-support. Per-leg history: `vault/journal/`._

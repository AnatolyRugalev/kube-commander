...
- [ ] **STORY-06j-2** Scrolling past the last log line re-arms follow — the same re-arm `G` gives, so a reader who scrolled back down to the newest line is following again without a keypress (`2026-08-15-logs-scroll-past-end-resumes-follow.md`)
      status: in-progress | owner: claude-opus | added: 2026-08-16
      notes: Second slice of STORY-06j. The current behaviour — only `nav.bottom` re-arms, pinned by `TestDownwardScrollDoesNotResumeFollowing` — sits at the end, paused. The fix re-engages follow when a down-scroll reaches the newest line; the test that pins the old rule is part of this slice's scope.
      → milestone: M5 · knowledge: decisions.md D268

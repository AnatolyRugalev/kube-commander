...
func TestHelpMapFullHelp(t *testing.T) {
	...
	// Registry has fourteen namespaces: nav.* then app.* then ns.* then resources.*
	// then mouse.* then sort.* then menu.* then actions.* then res.* (M3-02) then
	// logs.* (M3-06) then secret.* (M3-08a) then forwards.* (M3-13b) then search.*
	// (SEARCH-02b) then confirm.* (D132).
	if len(full) != 14 {
		t.Fatalf("FullHelp columns = %d; want 14", len(full))
	}
	...
	if cols := km.HelpMap().FullHelp(); len(cols) != 13 {
		t.Errorf("FullHelp after disabling nav.* columns = %d; want 13 (app + ns + resources + mouse + sort + menu + actions + res + logs + secret + forwards + search + confirm)", len(cols))
	}
}
